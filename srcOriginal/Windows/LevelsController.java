 ...
 * crea la transsion entre las mismas
 * activa o desectiva los controladores de cada escena
 * elimina los controladores de cada ecena
 * determina cuando muuere y el personaje pierde una vida
 * mueve el panel de vida coins y kunas de manera animada
 * bloque la entrada por teclado cuando se encuentra la transicion de escena
...
public class LevelsController extends JPanel implements Screem {
    ...
    /**
     * Timer donde ira de manera alternada por escena el controllador de cada escena
     */
     ...
     */
    public Scena1World scena1;
        /**
     * controllador de escena
     */
    public Scena2World scena2;
        /**
     * controllador de escena
     */
    public Scena3World scena3;
        /**
     * controllador de escena
     */
    public Scena4World scena4;
        /**
     * controllador de escena
     */
    public Scena5World scena5;
        /**
     * controllador de escena
     */
    public Scena6World scena6;
        /**
     * controllador de escena
     */
    public Scena7World scena7;
        /**
     * controllador de escena
     */
    public Scena8World scena8;
        /**
     * controllador de escena
     */
    ...
    public Personaje main;
        ...
        /**
     * controllador de la altira del suelo ( se esta eliminando su implementacion)
     */
     ...
     */
    public LevelsController(int x, int y, GameView viewLabel, GameOver panel,int initInt) {
...
//                    blockEntry = false;
                ...
                // subiendo
                if ((scena2.maluco.deading == true && sceneNumber == 7) || sceneNumber != 7 && moving == false && viewLabel.acumLive > 0) {

                    if (main.x >= (width * sceneNumber) - 250 && movements * -1 <= width * sceneNumber && main.orientation == 1) {
...
//            }
    ...
    public void hoStart() {
        if (sceneNumber == 1 && level1start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c1); // :( necesita el main :(
            main.block = false;
            level1start = true;
            level2start = false;
            level3start = false;
        ...
        if (sceneNumber == 2 && level2start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c2); // :( necesita el main :(
            controllerTimer.start();
            level1start = false;
            level2start = true;
            level3start = false;
        ...
        if (sceneNumber == 3 && level3start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c3); // :( necesita el main :(
            main.block = true;
            level1start = false;
            level2start = false;
            level3start = true;
        ...
        if (sceneNumber == 4 && level4start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c4); // :( necesita el main :(
            main.block = false;
            level1start = false;
            level2start = false;
            level3start = false;
        ...
        if (sceneNumber == 5 && level5start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c5); // :( necesita el main :(
            main.block = false;
            level1start = false;
            level2start = false;
            level3start = false;
        ...
        if (sceneNumber == 6 && level6start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c6); // :( necesita el main :(
            main.block = false;
            level1start = false;
            level2start = false;
            level3start = false;
        ...
        if (sceneNumber == 7 && level7start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c7); // :( necesita el main :(
            main.block = false;
            level1start = false;
            level2start = false;
            level3start = false;
        ...
        if (sceneNumber == 8 && level8start == false) {
            ...
            removeActions();
            controllerTimer = new Timer(17, c8); // :( necesita el main :(
            main.block = false;
            level1start = false;
            level2start = false;
            level3start = false;
...
//             viewLabel.determineLive();
        ...
        viewLabel.setRelative(-(width * (x - 1)));
        hoStart();

    }
             /**
              * Determian dondo empezara el personaje
     *Determina cual timer remover y cual iniciar
